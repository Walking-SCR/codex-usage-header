...
- **横向双列重置券卡片**：顶部蓝底轻量横幅展示可用重置券张数与余额（如 `额度余额：0.00`），去除冗余提示图标，下方双列并排陈列重置券到期时间；
...
- **智能在用账号感知与自动故障转移**：默认选中当前正在使用的账号；当账号 A 额度用完（如 5h 剩余 0%）且底层自动切换到账号 B 时，前端看板**自动同步切换并高亮选中账号 B**；
