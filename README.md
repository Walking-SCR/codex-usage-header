...
- **额度余额**：规范化格式化展示为两位小数（如 `0.00`、`1.20`），直观清晰；
- **重置券明细**：以精美票券卡片列出可用重置券（多张卡片保持 5px 严整间距），显示重置范围（如“完全重置（每周 + 5 小时）”）及精确到时区的到期时间（如“将于 10/4 13:06 GMT+8 到期”）；
- **自动刷新间隔**：保留在程序配置文件中，卡片不再展示刷新频率控件；
- 最近一次与客户端同步成功的时间。

### 4. 多模型额度扩展（Gemini AI Pro & 3P 模型）
除了官方 Codex 自身额度外，详情卡片下方独立追加了 **Gemini AI Pro** 额度看板：
- **多账号自由切换**：标题右侧自带精致的分段 Tab，支持在配置的多个 Google/Antigravity 账号之间无缝瞬时切换查看各自用量；
- 实时展示 **Gemini 5h**、**Gemini 7d**、**Claude & GPT 7d** 以及 **Claude & GPT 5h** 的配额剩余百分比、专属色阶进度条与重置倒计时；
...
- **多模型用量分布**：精准聚合 GPT（gpt-* / codex-* / o系列）、Gemini（gemini-*）及其他第三方模型的消耗量与百分比占比；
